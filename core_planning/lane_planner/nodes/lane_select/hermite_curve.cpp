...
void getPointAndVectorFromPose(const geometry_msgs::Pose &pose, Element2D *point,
  Element2D *vector) {
...
std::vector<autoware_msgs::Waypoint> generateHermiteCurveForROS(const geometry_msgs::Pose &start,
  ...
  for (uint32_t i = 0; i < result.size(); i++) {
    ...
    wp.pose.pose.position.z =
      (i == 0) ? start.position.z : (i == result.size() - 1) ? end.position.z : start.position.z <
      end.position.z ?
...
std::vector<Element2D> generateHermiteCurve(const Element2D &p0, const Element2D &v0,
  const Element2D &p1,
  ...
  while (divide < loop) {
    ...
    for (int32_t i = 0; i < divide; i++) {
      ...
      result.push_back(
        Element2D((p0.x * coeff_p0 + vlength * v0.x * coeff_v0 + p1.x * coeff_p1 + vlength * v1.x *
            coeff_v1),
