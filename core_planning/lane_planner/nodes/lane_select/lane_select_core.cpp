...
void LaneSelectNode::initForROS() {
  ...
  sub6_ = nh_.subscribe("/decision_maker/state", 1, &LaneSelectNode::callbackFromDecisionMakerState,
      this);
  // setup publisher
  ...
  private_nh_.param<int>("search_closest_waypoint_minimum_dt", search_closest_waypoint_minimum_dt_,
    int(5));
}
...
int32_t LaneSelectNode::getClosestLaneChangeWaypointNumber(const
...
void LaneSelectNode::createLaneForChange() {
  ...
  if ((static_cast<ChangeFlag>(cur_lane.waypoints.at(num_lane_change).change_flag) ==
      ChangeFlag::right &&
      right_lane_idx_ < 0) ||
  ...
  double dt_by_vel = current_velocity_.twist.linear.x * lane_change_target_ratio_ >
  ...
  for (uint32_t i = nghbr_clst_wp; i < nghbr_lane.waypoints.size(); i++) {
    if (i == nghbr_lane.waypoints.size() - 1 ||
      dt + dt_by_vel < getTwoDimensionalDistance(nghbr_lane.waypoints.at(
          nghbr_clst_wp).pose.pose.position,
  ...
  for (auto i = itr; i != nghbr_lane.waypoints.end(); i++) {
    if (getTwoDimensionalDistance(itr->pose.pose.position,
        i->pose.pose.position) < lane_change_interval_) {
...
void LaneSelectNode::changeLane() {
  ...
  } else if (std::get<2>(tuple_vec_.at(current_lane_idx_)) == ChangeFlag::left &&
    left_lane_idx_ != -1 &&
...
bool LaneSelectNode::getClosestWaypointNumberForEachLanes() {
  for (auto &el : tuple_vec_) {
    std::get<1>(el) = getClosestWaypointNumber(std::get<0>(el), current_pose_.pose,
...
int32_t LaneSelectNode::findMostClosestLane(const std::vector<uint32_t> idx_vec,
  ...
  for (const auto &el : idx_vec) {
    ...
    dist_vec.push_back(
      getTwoDimensionalDistance(p, std::get<0>(tuple_vec_.at(el)).waypoints.at(
          closest_number).pose.pose.position));
  }
...
void LaneSelectNode::findNeighborLanes() {
  ...
  for (uint32_t i = 0; i < tuple_vec_.size(); i++) {
    ...
    const geometry_msgs::Point &target_p = std::get<0>(tuple_vec_.at(i)).waypoints.at(
        target_num).pose.pose.position;
    geometry_msgs::Point converted_p = convertPointIntoRelativeCoordinate(target_p,
        current_closest_pose);
...
int32_t getClosestWaypointNumber(const autoware_msgs::Lane &current_lane,
  ...
  if (previous_number == -1) {
    ...
    if (distance_threshold <
      getTwoDimensionalDistance(current_lane.waypoints.at(previous_number).pose.pose.position,
        current_pose.position)) {
      ROS_WARN("Current_pose is far away from previous closest waypoint. Initilized...");
      return -1;
    }
    double ratio = 3;
    double dt = std::max(current_velocity.linear.x * ratio,
        static_cast<double>(search_closest_waypoint_minimum_dt));
  ...
  dist_vec.reserve(idx_vec.size());
  for (const auto &el : idx_vec) {
    double dt = getTwoDimensionalDistance(current_pose.position,
...
bool getLinearEquation(geometry_msgs::Point start, geometry_msgs::Point end, double *a, double *b,
  double *c) {
