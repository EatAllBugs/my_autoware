...
void PathVset::setDeceleration() {
  ...
  for (int i = 0; i < velocity_change_range; i++) {
    ...
    double waypoint_velocity = current_waypoints_.waypoints[g_closest_waypoint +
...
void PathVset::avoidSuddenBraking() {
  ...
  double temp1 = (g_current_vel - g_velocity_change_limit + 1.389) * (g_current_vel -
      g_velocity_change_limit + 1.389);
  double temp2 = 2 * g_decel * interval;
...
EControl vscanDetection() {
  ...
  for (int i = g_closest_waypoint; i < g_closest_waypoint + g_search_distance; i++) {
    ...
    decelerate_or_stop++;
    if (decelerate_or_stop > decelerate2stop_waypoints || (decelerate_or_stop >= 0 &&
        i >= g_path_dk.getSize() - 1) ||
    ...
    geometry_msgs::Point waypoint = calcRelativeCoordinate(g_path_dk.getWaypointPosition(i),
        g_localizer_pose.pose);
    tf::Vector3 tf_waypoint = point2vector(waypoint);
    tf_waypoint.setZ(0);
    int stop_point_count = 0;
    int decelerate_point_count = 0;
    for (pcl::PointCloud<pcl::PointXYZ>::const_iterator item = g_vscan.begin(); item != g_vscan.end();
      item++) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_point = nh.subscribe("vector_map_info/point", 1, &CrossWalk::pointCallback,
      &vmap);
  //----------------------------------------------------//
