...
void DecisionMakerNode::callbackFromLightColor(const
  ros::MessageEvent<autoware_msgs::TrafficLight const> &event) {
...
#define NUM_OF_SET_MISSION_COMPLETE_FLAG 3
    ...
    unsigned counter = 0;
    for (counter = 0;
      counter <= (wp_idx <= NUM_OF_SET_MISSION_COMPLETE_FLAG ? wp_idx : NUM_OF_SET_MISSION_COMPLETE_FLAG);
      counter++) {
