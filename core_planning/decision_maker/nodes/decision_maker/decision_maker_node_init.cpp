...
void DecisionMakerNode::createSubscriber(void) {
  ...
  Subs["change_flag"] = nh_.subscribe("/change_flag", 1,
      &DecisionMakerNode::callbackFromLaneChangeFlag, this);
}
void DecisionMakerNode::createPublisher(void) {
  ...
  Pubs["available_transition"] = private_nh_.advertise<std_msgs::String>("available_transition", 1,
      true);
  Pubs["stop_cmd_location"] = private_nh_.advertise<autoware_msgs::VehicleLocation>("stop_location",
      1, true);
  // for debug
  Pubs["target_velocity_array"] = nh_.advertise<std_msgs::Float64MultiArray>("target_velocity_array",
      1);
  Pubs["operator_help_text"] =
    private_nh_.advertise<jsk_rviz_plugins::OverlayText>("operator_help_text", 1, true);
}
...
void DecisionMakerNode::initVectorMap(void) {
  int _index = 0;
  const std::vector<CrossRoad> crossroads = g_vmap.findByFilter([](const CrossRoad & crossroad) {
    return true;
  });
  if (crossroads.empty()) {
  ...
  for (const auto &cross_road : crossroads) {
    ...
    g_vmap.findByFilter([&area](const Line & line) {
      return area.slid <= line.lid && line.lid <= area.elid;
    });
    for (const auto &line : lines) {
      const std::vector<Point> points =
      g_vmap.findByFilter([&line](const Point & point) {
        return line.bpid == point.pid;
      });
