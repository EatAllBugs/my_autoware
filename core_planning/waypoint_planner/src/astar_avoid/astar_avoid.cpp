...
AstarAvoid::AstarAvoid()
  ...
  private_nh_.param<double>("avoid_waypoints_velocity", avoid_waypoints_velocity_,
    10.0);
  ...
  //唯一的发布　安全的waypoints
  safety_waypoints_pub_ = nh_.advertise<autoware_msgs::Lane>("safety_waypoints",
      1, true);
  costmap_sub_ = nh_.subscribe("costmap", 1, &AstarAvoid::costmapCallback, this);
  current_pose_sub_ = nh_.subscribe("current_pose", 1,
      &AstarAvoid::currentPoseCallback, this);
  current_velocity_sub_ = nh_.subscribe("current_velocity", 1,
      &AstarAvoid::currentVelocityCallback, this);
  base_waypoints_sub_ = nh_.subscribe("base_waypoints", 1,
      &AstarAvoid::baseWaypointsCallback, this);
  closest_waypoint_sub_ = nh_.subscribe("closest_waypoint", 1,
      &AstarAvoid::closestWaypointCallback, this);
  obstacle_waypoint_sub_ = nh_.subscribe("obstacle_waypoint", 1,
      &AstarAvoid::obstacleWaypointCallback, this);
...
void AstarAvoid::currentPoseCallback(const geometry_msgs::PoseStamped &msg) {
  ...
  if (!enable_avoidance_) {
    ...
    current_pose_local_.pose = transformPose(
        current_pose_global_.pose, getTransform(costmap_.header.frame_id,
...
void AstarAvoid::currentVelocityCallback(const geometry_msgs::TwistStamped
  &msg) {
...
void AstarAvoid::run() {
  ...
  obstacle_waypoint_index_ = -1;
  // relaying mode at startup 开始是relay mode
  state_ = AstarAvoid::STATE::RELAYING;
  // start publish thread 发布线程
  ...
  while (ros::ok()) {
    ...
    bool avoid_velocity = (current_velocity_.twist.linear.x <
    ...
    if (state_ == AstarAvoid::STATE::RELAYING) {
      ...
      bool replan = ((ros::WallTime::now() - start_plan_time).toSec() >
          replan_interval_);
      if (!found_obstacle) {
      ...
      bool reached = (getLocalClosestWaypoint(avoid_waypoints_,
            current_pose_global_.pose, closest_search_size_) > end_of_avoid_index);
      if (reached) {
        ROS_INFO("AVOIDING -> RELAYING, Reached goal");
        state_ = AstarAvoid::STATE::RELAYING;
      } else if (found_obstacle && avoid_velocity) {
        bool replan = ((ros::WallTime::now() - start_avoid_time).toSec() >
            replan_interval_);
        if (replan) {
...
bool AstarAvoid::checkInitialized() {
  ...
  initialized = (current_pose_initialized_ && closest_waypoint_initialized_ &&
      base_waypoints_initialized_ &&
      (closest_waypoint_index_ >= 0));
  // check for avoidance mode, additionally
  if (enable_avoidance_) {
    initialized = (initialized && (current_velocity_initialized_ &&
...
bool AstarAvoid::planAvoidWaypoints(int &end_of_avoid_index) {
  ...
  int closest_waypoint_index = getLocalClosestWaypoint(avoid_waypoints_,
      current_pose_global_.pose, closest_search_size_);
  // update goal pose incrementally and execute A* search
  for (int i = search_waypoints_delta_;
    i < static_cast<int>(search_waypoints_size_); i += search_waypoints_delta_) {
    // update goal index
    int goal_waypoint_index = closest_waypoint_index + obstacle_waypoint_index_ + i;
    if (goal_waypoint_index >= static_cast<int>
...
void AstarAvoid::mergeAvoidWaypoints(const nav_msgs::Path &path,
  ...
  int closest_waypoint_index = getLocalClosestWaypoint(current_waypoints,
      current_pose_global_.pose, closest_search_size_);
  ...
  for (const auto &pose : path.poses) {
    ...
    wp.pose.pose = transformPose(pose.pose,
        getTransform(avoid_waypoints_.header.frame_id, pose.header.frame_id));
    wp.pose.pose.position.z =
      current_pose_global_.pose.position.z;  // height = const
    wp.twist.twist.linear.x = avoid_waypoints_velocity_ /
      3.6;       // velocity = const
    avoid_waypoints_.waypoints.push_back(wp);
  }
  // add waypoints after goal index
  for (int i = end_of_avoid_index;
    i < static_cast<int>(current_waypoints.waypoints.size()); ++i) {
    avoid_waypoints_.waypoints.push_back(current_waypoints.waypoints.at(i));
  }
...
void AstarAvoid::publishWaypoints() {
  ...
  while (!terminate_thread_) {
    ...
    for (int i = 0; i < safety_waypoints_size_; ++i) {
      int index = getLocalClosestWaypoint(current_waypoints,
          current_pose_global_.pose, closest_search_size_) + i;
      if (index < 0 ||
        static_cast<int>(current_waypoints.waypoints.size()) <= index) {
        break;
      }
...
tf::Transform AstarAvoid::getTransform(const std::string &from,
  const std::string &to) {
...
int AstarAvoid::getLocalClosestWaypoint(const autoware_msgs::Lane &waypoints,
  ...
  // search in limited area based on prev_index
    ...
    int end_index = std::min(prev_index + search_size / 2,
    ...
    local_waypoints.waypoints = std::vector<autoware_msgs::Waypoint>(start_itr,
        end_itr);
    // get closest waypoint in neighborhood waypoints
    int closest_local_index = start_index + getClosestWaypoint(local_waypoints,
        pose);
