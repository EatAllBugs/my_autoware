...
void HevCnt::UpdateDriveState(REP_DRIVE_INFO_INDEX index) {
  switch (index) {
    ...
    case REP_GAS_PEDAL:
      _hevCnt->GetGasStroke((int &)_drvInf.actualPedalStr, (int &)_drvInf.targetPedalStr,
    ...
    case REP_BRAKE_PEDAL_FROMOBD:
      ...
      break;
    case REP_SHIFT_POS:
      _hevCnt->GetShiftMode((int &)_drvInf.actualShift, (int &)_drvInf.targetShift,
    ...
    case REP_BRAKE_STATUS:
      _hevCnt->GetBrakeStatus((unsigned char &)_brakeInf.brakeLamp,
        (unsigned char &)_brakeInf.blinkerLeft, (unsigned char &)_brakeInf.blinkerRight,
...
void HevCnt::ReceiveVersion(char c0, char c1, char c2, char c3, char c4, char c5, char c6,
  char c7) {
...
void HevCnt::rcvTime(char *date) {
  getDate();
  sprintf(date, "%d/%d/%d/%d:%d:%d.%ld", _s_time->tm_year + 1900, _s_time->tm_mon + 1,
    _s_time->tm_mday,
