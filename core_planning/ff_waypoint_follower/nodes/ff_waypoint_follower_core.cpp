...
FFSteerControl::FFSteerControl() {
  ...
  ROS_INFO("Origin : x=%f, y=%f, z=%f", transform.getOrigin().x(), transform.getOrigin().y(),
  ...
  // define subscribers.
  sub_initialpose 		= nh.subscribe("/initialpose", 		100, &FFSteerControl::callbackGetInitPose,
      this);
  if (m_CmdParams.statusSource != SIMULATION_STATUS) {
    sub_current_pose 	= nh.subscribe("/current_pose", 		100, &FFSteerControl::callbackGetCurrentPose,
        this);
  }
  ...
  sub_behavior_state 		= nh.subscribe("/current_behavior",	10,
      &FFSteerControl::callbackGetBehaviorState, 	this);
  sub_current_trajectory 	= nh.subscribe("/final_waypoints", 	10,
...
void FFSteerControl::callbackGetInitPose(const geometry_msgs::PoseWithCovarianceStampedConstPtr
  &msg) {
...
void FFSteerControl::callbackGetCurrentVelocity(const geometry_msgs::TwistStampedConstPtr &msg) {
  ...
  if (msg->twist.linear.x != 0) {
    m_CurrVehicleStatus.steer = atan(m_CarInfo.wheel_base * msg->twist.angular.z /
        msg->twist.linear.x);
  }
...
//				msg->pose.pose.position.y,msg->pose.pose.position.z , tf::getYaw(msg->pose.pose.orientation));
    ...
    if (msg->twist.twist.linear.x != 0) {
      m_CurrVehicleStatus.steer = atan(m_CarInfo.wheel_base * msg->twist.twist.angular.z /
...
void FFSteerControl::displayFollowingInfo(PlannerHNS::WayPoint &curr_pose,
  PlannerHNS::WayPoint &perp_pose, PlannerHNS::WayPoint &follow_pose) {
  ...
  m1.pose.orientation = tf::createQuaternionMsgFromYaw(UtilityHNS::UtilityH::SplitPositiveAngle(
        curr_pose.pos.a));
  ...
  m3.pose.orientation = tf::createQuaternionMsgFromYaw(UtilityHNS::UtilityH::SplitPositiveAngle(
        follow_pose.pos.a));
...
PlannerHNS::BehaviorState FFSteerControl::ConvertBehaviorStateFromAutowareToPlannerH(
    ...
    if (currMessage.state != PlannerHNS::INITIAL_STATE && (bInitPos || bNewCurrentPos)) {
      ...
      if (m_CmdParams.statusSource == CONTROL_BOX_STATUS) {
        ...
        if (m_pComm && m_pComm->IsAuto()) {
          ...
          control_box_status.pose.pose.orientation = tf::createQuaternionMsgFromYaw(
        ...
        vehicle_status.twist.angular.z = (vehicle_status.twist.linear.x / m_CarInfo.wheel_base) * tan(
        ...
        pose.pose.orientation = tf::createQuaternionMsgFromYaw(UtilityHNS::UtilityH::SplitPositiveAngle(
              m_CurrentPos.pos.a));
...
//				pub_VehicleStatus.publish(vehicle_status);
       ...
       */
      bool bNewPath = false;
      if (PlannerHNS::PlanningHelpers::CompareTrajectories(m_FollowingTrajectory,
          m_State.m_Path) == false && m_State.m_Path.size() > 0) {
...
//				cout << "Path is Updated in the controller .. " << m_State.m_Path.size() << endl;
      ...
      m_PrevStepTargetStatus = m_PredControl.DoOneStep(dt, currMessage, m_FollowingTrajectory,
...
//					<< ", " << m_PrevStepTargetStatus.shift << ")" << endl;
      ...
      if (m_CmdParams.statusSource == CONTROL_BOX_STATUS) { //send directly to ZMP control box
        ...
        if (m_pComm && m_pComm->IsAuto()) {
          ...
          cout << "Sending Data to Control Box (" << m_PrevStepTargetStatus.steer << ", " <<
        ...
        cout << "Send Data To Robot : Max Speed=" << m_CarInfo.max_speed_forward << ", actual = " <<
          m_PrevStepTargetStatus.speed << endl;
