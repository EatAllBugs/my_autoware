 ...
 */
void RayGroundFilter::ConvertXYZIToRTZColor(const
  ...
  for (size_t i = 0; i < in_cloud->points.size(); i++) {
    ...
    auto theta          = (float) atan2(in_cloud->points[i].y,
        in_cloud->points[i].x) * 180 / M_PI;
    if (theta < 0) {
      theta += 360;
    }
    auto radial_div     = (size_t) floor(theta / radial_divider_angle_);
    auto concentric_div = (size_t) floor(fabs(radius /
 ...
 */
void RayGroundFilter::ClassifyPointCloud(std::vector<PointCloudXYZIRTColor>
 ...
 */
void RayGroundFilter::ClipCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr
  in_cloud_ptr,
 ...
 */
void RayGroundFilter::ExtractPointsIndices(const
 ...
 */
void RayGroundFilter::RemovePointsUpTo(const
  pcl::PointCloud<pcl::PointXYZI>::Ptr in_cloud_ptr,
...
void RayGroundFilter::CloudCallback(const sensor_msgs::PointCloud2ConstPtr
  &in_sensor_cloud) {
  node_status_pub_ptr_->NODE_ACTIVATE();
  node_status_pub_ptr_->CHECK_RATE("/topic/rate/points_raw/slow", 8, 5, 1,
    "topic points_raw subscribe rate low.");
  pcl::PointCloud<pcl::PointXYZI>::Ptr current_sensor_cloud_ptr(
    new pcl::PointCloud<pcl::PointXYZI>);
  pcl::fromROSMsg(*in_sensor_cloud, *current_sensor_cloud_ptr);
  pcl::PointCloud<pcl::PointXYZI>::Ptr clipped_cloud_ptr(new
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr ground_cloud_ptr(new
    pcl::PointCloud<pcl::PointXYZI>);
...
void RayGroundFilter::Run() {
  ...
  node_handle_.param("concentric_divider_distance", concentric_divider_distance_,
    0.01);//0.1 meters default
  ...
  config_node_sub_ = node_handle_.subscribe("/config/ray_ground_filter", 1,
      &RayGroundFilter::update_config_params, this);
  groundless_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>
    (no_ground_topic, 2);
  ground_points_pub_ = node_handle_.advertise<sensor_msgs::PointCloud2>
    (ground_topic, 2);
  ROS_INFO("Ready");
  ros::spin();
}
