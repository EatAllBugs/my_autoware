...
void setOccupancyGridMap(nav_msgs::OccupancyGrid *map,
  ...
  map->info.origin.position.x = transform.getOrigin().x() -
    (g_map_size_x / 2) * g_resolution;
  map->info.origin.position.y = transform.getOrigin().y() -
    (g_map_size_y / 2) * g_resolution;
...
void createCostMap(const sensor_msgs::LaserScan &scan,
  ...
  for (size_t i = 0; i < scan.ranges.size(); i++) {
    ...
    // Obstacle
    int global_index =
      precasted_grids[precasted_index][obstacle_index].calcGlobalIndex(transform);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber laserscan_sub = nh.subscribe(g_scan_topic, 1,
      laserScanCallback);
